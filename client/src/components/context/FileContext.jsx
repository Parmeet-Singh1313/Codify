...
export const useFileSystem = () => {
    const context = useContext(FileContext); if (!context) { throw new Error("useFileSystem must be used within FileContextProvider"); } return context;
}
function FileContextProvider({ children, routerParams, routerLocation }) {
    const { currentUser } = useAppContext();
    const { socket } = useSocket(); const { setUsers, drawingData } = useAppContext();
    // Function to get roomId, prioritizing URL params
    const getRoomId = () => {
        const pathname = window.location.pathname;
        const match = pathname.match(/\/editor\/([^/]+)/);
        return match ? match[1] : currentUser?.roomId || 'default';
    };

    const [fileStructure, setFileStructure] = useState(() => {
        const currentRoomId = getRoomId();
        const stored = localStorage.getItem(`fileStructure_${currentRoomId}`);
        return stored ? JSON.parse(stored) : initialFileStructure;
    });

    const [openFiles, setOpenFiles] = useState(() => {
        const currentRoomId = getRoomId();
        const stored = localStorage.getItem(`openFiles_${currentRoomId}`);
        return stored ? JSON.parse(stored) : [];
    });

    const [activeFile, setActiveFile] = useState(() => {
        const currentRoomId = getRoomId();
        const stored = localStorage.getItem(`activeFile_${currentRoomId}`);
        return stored ? JSON.parse(stored) : null;
    });

    // Effect to handle Local Storage
    useEffect(() => {
        const currentRoomId = getRoomId();
        if (!currentRoomId) return;

        // Save to Local Storage
        localStorage.setItem(`fileStructure_${currentRoomId}`, JSON.stringify(fileStructure));
        localStorage.setItem(`openFiles_${currentRoomId}`, JSON.stringify(openFiles));
        localStorage.setItem(`activeFile_${currentRoomId}`, JSON.stringify(activeFile));
    }, [fileStructure, openFiles, activeFile]);

    // Effect to load data when room changes
    useEffect(() => {
        const handleLocationChange = () => {
            ...
            const storedFileStructure = localStorage.getItem(`fileStructure_${currentRoomId}`);
            const storedOpenFiles = localStorage.getItem(`openFiles_${currentRoomId}`);
            const storedActiveFile = localStorage.getItem(`activeFile_${currentRoomId}`);

            if (storedFileStructure) {
                setFileStructure(JSON.parse(storedFileStructure));
            } else {
                setFileStructure(initialFileStructure);
            }

            if (storedOpenFiles) {
                setOpenFiles(JSON.parse(storedOpenFiles));
            } else {
                setOpenFiles([]);
            }

            if (storedActiveFile) {
                setActiveFile(JSON.parse(storedActiveFile));
            } else {
                setActiveFile(null);
            }
        };
    ...
    const deleteDirectory = useCallback(
        (dirId, sendToSocket = true) => {
            const deleteFromDirectory = (directory) => {
                if (directory.id === dirId) {
                    return null; // Remove the directory
                } else if (directory.children) {
            ...
            setFileStructure((prevFileStructure) => {
                const newStructure = deleteFromDirectory(prevFileStructure);
                return newStructure;
            });
